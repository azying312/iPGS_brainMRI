...
#print(paste("sqc_f:", sqc_f))
#fields_f <- file.path("MRI_Fields.tsv")
#SQL_dump_f <- file.path("SQL_dump.tsv.gz")
...
pc_plot <- train_with_phenotypes_long %>%
  mutate(GBE_ID = as.factor(GBE_ID)) %>%
  ggplot(aes(x=PC1, y=PC2, color=GBE_ID))+
  geom_point(alpha = 0.7, size = 2) +
  geom_jitter(alpha = 0.7, size = 2, width = 0.05, height = 0.05) +
  theme_minimal() +
  labs(title="PC1 v. PC2 Phenotypes", xlab="PC1", ylab="PC2", color="Phenotype")+
